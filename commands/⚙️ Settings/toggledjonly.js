    ...
    try{
      ...
      if (!cmd)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle("<:713400011714723932:891458936656515092> Please add a cmd!")
      ...
      if(musiccmds.join(" ").toLowerCase().split(" ").includes(args[0].toLowerCase())){
          ...
          if(client.settings.get(message.guild.id, `djonlycmds`).join(" ").toLowerCase().split(" ").includes(args[0].toLowerCase())){
            try{
              ...
              return message.channel.send(new MessageEmbed()
                ...
                .setFooter(es.footertext, es.footericon)
                .setTitle(`<a:762633663266553857:891459306195664898> Set Cmd \`${args[0]}\` to NOT DJ ONLY`)
            ...
            try{
              ...
              return message.channel.send(new MessageEmbed()
                .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                .setFooter(es.footertext, es.footericon)
                .setTitle(`<a:762633663266553857:891459306195664898> Set Cmd \`${args[0]}\` to DJ ONLY`)
                .setDescription(`All Dj-ONLY-CMDS:\n> \`${client.settings.get(message.guild.id, `djonlycmds`).sort(function(a, b){if(a < b) { return -1; }if(a > b) { return 1; }  return 0;}).join("`, `")}\``)
              );
            }catch (e){
              console.log(String(e.stack).red);
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> Could not find Music Command \`${args[0]}\``)
        );
      }
    } catch (e) {
