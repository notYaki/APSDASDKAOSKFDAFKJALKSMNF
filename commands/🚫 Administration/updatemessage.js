    ...
    try {
      ...
      if (!channel || channel == null || !channel.id || channel.id == 0)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> You didn't ping a Valid Channel`)
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> You didn't add a Valid Message ID`)
          .setDescription(`Usage: \`${prefix}copymessage <#Channel> <Message_ID>\``)
        );
      ...
      channel.messages.fetch(id).then(msg=>{
        if(msg.content && !msg.embeds[0]){
          ...
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> There is nothing to update`)
        );
        }
        if(msg.embeds[0]){
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:762633663266553857:891459306195664898> Updated the Embed in:`)
