        ...
        msg.channel.awaitMessages(m => m.author.id == message.author.id, {
          ...
          if (collected.first().content.toLowerCase().includes("yes")) {
            ...
            role.delete(`${message.author.tag} Requested a Role delete`)
              .then(r => {
                message.channel.send(new MessageEmbed()
                  .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                  .setFooter(es.footertext, es.footericon)
                  .setTitle(`<a:762633663266553857:891459306195664898> Deleted \`${r.name}\` and removed it from \`${membersize} Members\``)
                );
            ...
            return message.channel.send(new MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<:713400011714723932:891458936656515092> You did not add **__yes__**`)
