    ...
    try {
      ...
      if (!role || role == null || role == undefined|| role.name == null || role.name == undefined)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> please ping a ROLE!`)
          .setDescription(` Usage: \`${prefix}removerole @USER @ROLE\``)
        );
      if (member.roles.highest.position >= message.member.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> I cannot remove that Role from this Member, because he is higher/Equal to your Rang Position!`)
        );
      if(message.member.roles.highest.position <= role.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> I cannot remove that Role from this Member, because it's higher then your highest ROLE!`)
        );
      if(!member.roles.cache.some(r=>r.id == (role.id)))
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> I cannot remove that Role from this Member, because he doesn't have it!`)
        );
      var ge = false;
      ...
      if(ge) return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)
      ...
      message.channel.send(new MessageEmbed()
        ...
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<a:762633663266553857:891459306195664898> removed \`${role.name}\` from \`${member.user.tag}\``)
      );
