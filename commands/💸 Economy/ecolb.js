    ...
    try {
      ...
      ensure_economy_user(client, message.guild.id, user.id)
        ...
        for(let i = 0; i < sorted.length; i++){
          ...
          const tuser = await client.users.fetch(data.user);
          if(!tuser) continue;
        ...
        const queueEmbed = await message.channel.send(
          ...
          embeds[currentPage].setFooter(`You are: Rank #${yourrank} ・ Page: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true}))
          ...
          try {
            if (reaction.emoji.name === reactionemojis[1] || reaction.emoji.id === reactionemojis[1]) {
              if (currentPage < embeds.length - 1) {
                currentPage++;
                queueEmbed.edit({embed: embeds[currentPage].setFooter(`You are: Rank #${yourrank} ・ Page: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true}))});
              } else {
                currentPage = 0
                queueEmbed.edit({ embed: embeds[currentPage].setFooter(`You are: Rank #${yourrank} ・ Page: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true}))});
              }
