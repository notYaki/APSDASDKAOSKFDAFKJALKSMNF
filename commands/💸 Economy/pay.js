  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if(!payamount)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:713400011714723932:891458936656515092> You didn't add the payamount`)
        .setDescription(`Usage: \`${prefix}pay <@USER> <Amount>\`\n\n\Example: \`${prefix}pay <@442355791412854784> 42069\``)
      );
    ...
    return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:713400011714723932:891458936656515092> You can't pay a negative Amount of Money or no Money, to ${topay}`)
      );
    ...
    if(payamount > data.balance)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:713400011714723932:891458936656515092> You can't pay more Money than you have in your **👛 Pocket (\`${data.balance} 💸\`)**`)
      );
    ...
    return message.reply(new MessageEmbed()
      ...
      .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
      .setTitle(`<a:762633663266553857:891459306195664898> **You payed \`${payamount} 💸\` to \`${topay.tag}\`**`)
