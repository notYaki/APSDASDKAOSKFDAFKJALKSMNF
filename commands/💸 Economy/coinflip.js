    ...
    try {
      ...
      if (!flip || !['heads', 'tails'].includes(flip)) 
        return message.reply(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> Specify the \`flip-result\`, it should be a number between \`heads\`-\`tails\``)
      ...
      if (!amount) 
        return message.reply(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> Specify the **amount of \`Coins 💸\`** you want to gamble!`)
          .setDescription(`Usage: \`${prefix}coinflip <roll-result> <Gamble-Amount>\`\n\n\Example: \`${prefix}coinflip heads 100\``)
        );
      if (data.balance < amount) return message.reply(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<:713400011714723932:891458936656515092> You can't gamble more Money than you have in your **👛 Pocket (\`${data.balance} 💸\`)**`)
      );
