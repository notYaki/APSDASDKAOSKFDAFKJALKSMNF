  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (!client.settings.get(message.guild.id, "ECONOMY")) {
      return message.channel.send(new MessageEmbed()
        ...
        .setTitle(`<a:865963806483808256:896694904019894282> THIS COMMAND IS CURRENTLY DISABLED`)
    ...
    try {
      ...
      if (user.bot) return message.reply("<a:865963806483808256:896694904019894282> **A Discord Bot can not have Economy!**")
...
🟫 **\`Dirthut [150 💸]\`** | ${p2b(150)}`
      ...
      if (amountofbuy == 0)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`<:713400011714723932:891458936656515092> You cannot buy 0 Items`)
      ...
      if (!prize)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag + " | ❌ .. Unable to buy | ✅ ... Possible to buy", user.displayAvatarURL({ dynamic: true }))
          .setTitle(`<:713400011714723932:891458936656515092> This Item seems to not exist! Those Items are available:`)
          .setDescription(`👛 You have (\`${data.balance} 💸\`) in your Pocket \n\n🧸 **You have \`${items} Items\` with a value of: \`${itemsvalue} 💸\`**\n\n**__How to buy an Item?__**\n> \`${prefix}buy <ITEMNAME> <AMOUNT>\``)
.addField("✏️ Useables", ">>> " + 
`✏️ **\`Pensil [10 💸]\`** | ${p2b(10)}
🖊️ **\`Pen [20 💸]\`** | ${p2b(20)}
...
🟫 **\`Dirthut [150 💸]\`** | ${p2b(150)}`
      ...
      if (endprize > data.balance)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({ dynamic: true }))
          .setTitle(`<:713400011714723932:891458936656515092> You can't buy **${nFormatter(amountofbuy)} ${args[0]}** because it costs more Money (\`${nFormatter(endprize)} 💸\`) then you have in your **👛 Pocket (\`${nFormatter(data.balance)} 💸\`)**`)
      ...
      return message.channel.send(new MessageEmbed()
        .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        .setFooter(user.tag, user.displayAvatarURL({ dynamic: true }))
        .setTitle(`<a:762633663266553857:891459306195664898> **Successfully bought ${nFormatter(amountofbuy)} ${args[0]} for \`${nFormatter(endprize)} 💸\`**`)
