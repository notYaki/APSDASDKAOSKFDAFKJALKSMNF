    ...
    try {
      ...
      if (!args[0])
        return message.channel.send(new MessageEmbed()
          ...
          .setFooter(es.footertext, es.footericon)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:713400011714723932:891458936656515092> Please add your method!`)
      ...
      if (args[0].toLowerCase() === `song` || args[0].toLowerCase() === `track` || args[0].toLowerCase() === `s` || args[0].toLowerCase() === `t`) {
        ...
        let embed = new MessageEmbed()
          .setTitle(`<a:762633663266553857:891459306195664898> ${emoji.msg.repeat_mode} Changed Track loop to: ${player.trackRepeat ? `${emoji.msg.disabled} disabled` : `${emoji.msg.enabled} active`}`)
