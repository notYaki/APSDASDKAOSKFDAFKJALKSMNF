    ...
    try{
      ...
      if (Number(args[0]) < 0 || Number(args[0]) >= player.queue.current.duration / 1000)
        return message.channel.send(new MessageEmbed()
          .setFooter(es.footertext, es.footericon)
          .setColor(es.wrongcolor)
          .setTitle(`<:713400011714723932:891458936656515092> You may seek from \`0\` - \`${player.queue.current.duration}\``)
        );
      //seek to the position
      player.seek(Number(args[0]) * 1000);
      //send success message
      return message.channel.send(new MessageEmbed()
        .setTitle(`<a:762633663266553857:891459306195664898> Seeked song to: ${format(Number(args[0]) * 1000)}`)
