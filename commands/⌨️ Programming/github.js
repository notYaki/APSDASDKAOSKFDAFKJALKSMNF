		...
		try {
			...
			if (!repo)
				return message.channel.send({embed: new MessageEmbed()
					...
					.setTitle(`<:713400011714723932:891458936656515092> You didn't provide a Repository`)
			...
			if (!username || !repository) 
				return message.channel.send({embed: new MessageEmbed()
					.setColor(es.wrongcolor)
					.setFooter(es.footertext, es.footericon)
					.setTitle(`<:713400011714723932:891458936656515092> Repository must be in the form \`username/repository\``)
					.setDescription(`Usage: \`${prefix}github <REPO-LINK>\`\nExample: \`${prefix}github https://github.com/Ÿακι ジ#4635/Codes/\``)
				});
			...
			if (!body) 
				return message.channel.send({embed: new MessageEmbed()
					.setColor(es.wrongcolor)
					.setFooter(es.footertext, es.footericon)
					.setTitle(`<:713400011714723932:891458936656515092> Could not fetch that repo`)
