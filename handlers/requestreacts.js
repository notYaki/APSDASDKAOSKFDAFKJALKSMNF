      ...
      try {
        ...
        if (player && channel.id !== player.voiceChannel)
          return message.channel.send(new MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(es.footertext, es.footericon)
            .setTitle("<:713400011714723932:891458936656515092> I am already playing somewhere else!")
        ...
        if (channel.id !== db.voicechannel) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`You need to be in the: \`${message.guild.channels.cache.get(db.voicechannel).name}\` VoiceChannel`));
        ...
        switch (reaction.emoji.name) {
          ...
          case "♾":
            ...
            //Send Success Message
            message.channel.send(new MessageEmbed()
              .setTitle(`<a:762633663266553857:891459306195664898> ${player.get("autoplay") ? `\`✔️ Enabled\`` : `\`❌ Disabled\``} Autoplay`)
              .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
              .setFooter(es.footertext, es.footericon)
            ).then(async msg => {
              try {
