      ...
      client.manager
        .on("playerCreate", async (player) => {
            ...
            try {
              ...
              embed.addField(`${client.user.username} Powered by Ÿακι ジ#4635`, `**[Invite their Public Bot](https://discord.com/api/oauth2/authorize?client_id=902388483568599040&permissions=8&scope=bot) •  [Support Server](https://discord.gg/Qr65p2haQQ)**`)
            } catch {}
        ...
        .on("playerMove", async (player, oldChannel, newChannel) => {
          let es = client.settings.get(player.guild, "embed") || ee;
          if (!newChannel) {
              var embed = new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon);
              ...
              client.channels.cache.get(player.textChannel).send(embed).then(msg => {
                ...
                collector.on(`collect`, async (reaction, user) => {
                  try {
                    ...
                    if (!channel) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<:713400011714723932:891458936656515092> You need to join a voice channel.`));
                    //get the lavalink erela.js player information
                    var player = client.manager.players.get(message.guild.id);
                    //if no player return
                    if(!player || player == undefined) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<:713400011714723932:891458936656515092> I am not playing anything in this Server`));
                    //if there is a player and the user is not in the same channel as the Bot return information message
                    if (player && channel.id !== player.voiceChannel) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<:713400011714723932:891458936656515092> I am already playing somewhere else!`).setDescription(`You can listen to me in: ${message.guild.channels.cache.get(player.VoiceChannel).name}`));
                    ...
                    switch (reactionemoji) {
                      ...
                      case String(emoji.react.previous_track):
                        ...
                        if (!player.queue.previous || player.queue.previous === null)
                          return message.channel.send(new MessageEmbed()
                            .setColor(es.wrongcolor)
                            .setFooter(es.footertext, es.footericon)
                            .setTitle(`<:713400011714723932:891458936656515092> There is no previous song yet!`)
                          ).then(msg => {
                            if(msg && msg.deletable) msg.delete({
                                timeout: 4000
                              }).catch(e => console.log("couldn't delete message this is a catch to prevent a crash".grey));
                          ...
                          if (!player.get(`vote-${user.id}`)) {
                            ...
                            if (voteamount <= Number(player.get("votes"))) {
                              message.channel.send(new MessageEmbed()
                                .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                                .setFooter(es.footertext, es.footericon)
                                .setTitle(`<a:762633663266553857:891459306195664898> Added your Vote!`)
                      ...
                      case String(emoji.react.autoplay_mode):
                        ...
                        //Send Success Message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`<a:762633663266553857:891459306195664898> ${player.get("autoplay") ? `${emoji.msg.enabled} Enabled` : `${emoji.msg.disabled} Disabled`} Autoplay`)
                          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                          .setFooter(es.footertext, es.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
