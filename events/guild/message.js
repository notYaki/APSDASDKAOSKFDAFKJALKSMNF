        ...
        if (!guild_settings.botchannel.includes(message.channel.id) && !message.member.hasPermission("ADMINISTRATOR")) {
          //create the info string
            ...
            return message.channel.send(new Discord.MessageEmbed()
              ...
              .setTitle(`<:713400011714723932:891458936656515092> Error | Not in the Bot Chat!`)
        ...
        if(cmd.name.toLowerCase() === message.content.slice(prefix.length).split(" ")[0]){
          customcmd = true;
            ...
            return message.channel.send(new MessageEmbed()
                ...
                message.channel.send(cmd.output)
            }
        }
    }
    ...
    if (command && !customcmd){
        if(command.length == 0) {
          if(guild_settings.unkowncmdmessage){
            message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<:713400011714723932:891458936656515092> Unkown command, try: **\`${prefix}help\`**`)
        ...
        if (timestamps.has(message.author.id)) { //if the user is on cooldown
          ...
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            return message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setTitle(`<:713400011714723932:891458936656515092> Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`)
      ...
      try{
        ...
        //IF A FREEBOT RETURN
        let disablecmds = ["setup-radio", "playlist", "autoplay", "youtubetogether", "addroletoeveryone", "giveaway", "blacklist","tiktok", "setup-twitter", "anti", "aichat", "counter", "customcommand"]
        ...
        if(disablecmds.some(i => String(command.name).toLowerCase().includes(i.toLowerCase())) || 
          ...
          if(require('path').resolve(__dirname).split("/")[3].includes("FREEBOT_")){
            not_allowed = true;
            return message.channel.send(new MessageEmbed()
              .setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)
              .setTitle(`<:713400011714723932:891458936656515092> **THIS COMMAND IS PREMIUM ONLY** <a:nitro:833402717506502707>`)
        ...
        if(command.memberpermissions) {
          if (!message.member.hasPermission(command.memberpermissions)) {
            ...
            message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle("<:713400011714723932:891458936656515092> Error | You are not not_allowed to run this command!")
                ...
                //if not a DJ and not a nAdmin
                  ...
                  if(!player){
                    ...
                    not_allowed = true;
                      ...
                      .setTitle("<:713400011714723932:891458936656515092> Error | You are not allowed to run this command!")
            ...
            if(command.parameters.activeplayer){
              ...
              if (!mechannel){
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setFooter(ee.footertext, ee.footericon)
                  .setTitle(`<:713400011714723932:891458936656515092> I am not connected to a Channel`)
                );
              }
            }
            //if no previoussong
        ...
        return message.channel.send(new Discord.MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle("<:713400011714723932:891458936656515092> Something went wrong while, running the: `" + command.name + "` command")
